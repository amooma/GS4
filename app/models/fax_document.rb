class FaxDocument < ActiveRecord::Base
	...
	def to_raw( filename = nil, original_filename = 'fax.pdf' )
		...
		#OPTIMIZE Escape shell arguments.
	...
	def to_pdf
		...
		return pdf_file if (File.exists?( pdf_file ))
	end
	...
	def to_thumbnail
		...
		return thumbnail_file if (File.exists?( thumbnail_file ))
	end
	
	def raw_file_path
		raw_suffix = File.basename( Configuration.get( :fax_file_suffix, '.tif' ))
		raw_file = File.expand_path( "#{Configuration.get( :fax_files_directory )}/#{self.raw_file}#{raw_suffix}" )
		return raw_file if (File.exists?( raw_file ))
	end

#TODO Add this file in the Debian package.
...
class FaxMailer < ActionMailer::Base
	...
	def new_fax_document( fax_document )
		...
		if @user
			...
			mail(
				:to => "#{@user.gn} #{@user.sn} <#{@user.email}>",
				:subject => t(:fax_email_subject, :source => @fax_document.source, :destination => @fax_document.destination )
			)
		end
	end
end
